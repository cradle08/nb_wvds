...
//  set value for baseline ,avoid reboot and drop origin data
//  for(i=0;i<3;i++)
//  {
...
//  set value for baseline, avoid reboot and drop data
//  for(i=0;i<3;i++)
//  {
...
void Thershold_Init_By_Gain()
{
  ...
  signal.b_status = 0;
  signal.last_status = 3; //... init
  signal.values_index = 0;
  for(k = 0; k < VALUES_LEN; k++) //...
  {
    signal.values[k] = 0;
  }
...
uint8_t check_values()
{
  uint8_t i = 0;
  uint8_t status[VALUES_LEN] = {0};

  for(i = 0; i < VALUES_LEN; i++)
  {
    if(signal.values[i] == EN_LEAVE)
      status[EN_LEAVE]++;
    else if(signal.values[i] == EN_PARK)
        status[EN_PARK]++;
    else if(signal.values[i] == EN_INIT)
        status[EN_INIT]++;
    else if(signal.values[i] == EN_BIG_MAG)
        status[EN_BIG_MAG]++;
    else if(signal.values[i] == EN_INIT_FAILL)
        status[EN_INIT_FAILL]++;   
  }
...
//#endif
 ...
 //   re_value = signal.status;
   if(signal.values_index >= 5)  signal.values_index = 0;
   signal.values[signal.values_index++] = signal.status;
   re_value = check_values();
...
void Set_His_Value()
{
  ...
  signal.last_status = signal.status;
}
