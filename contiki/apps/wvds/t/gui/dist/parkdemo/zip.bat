...
7z a %NAME%.zip %NAME% -x!%NAME%\outdata\* -x!%NAME%\log\* -x!%NAME%\%NAME%.vbs -x!%NAME%\%NAME%.vbs2exe -x!%NAME%\zip.bat
goto end

:rar
set PATH=C:\Program Files\WinRAR;C:\Program Files (x86)\WinRAR
Rar a -x%NAME%\outdata\* -x%NAME%\log\* -x%NAME%\%NAME%.vbs -x%NAME%\%NAME%.vbs2exe -x%NAME%\zip.bat %NAME%.rar %NAME%
goto end
