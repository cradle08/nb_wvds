...
echo "* Copied as ${PROD}/activator.txt"
cp Exe/activator.txt ${PROD}/

echo "* Update version info in ${PROD}/version.csv"
verdate=$(git log -n 1 --pretty=format:'%ai' $verdir | sed 's/ .*//')
digest=$(sha256sum ${PROD}/activator.txt | awk '{print $1;}')
echo "-------------------"
cd ${PROD} && python $VERPY activator.txt ${verno} ${commit} ${verdate} ${digest}
echo "-------------------"
