...
echo "* Copied as ${PROD}/updater.txt"
cp Exe/updater.txt ${PROD}/

echo "* Update version info in ${PROD}/version.csv"
verdate=$(git log -n 1 --pretty=format:'%ai' $verdir | sed 's/ .*//')
digest=$(sha256sum ${PROD}/updater.txt | awk '{print $1;}')
echo "-------------------"
cd ${PROD} && python $VERPY updater.txt ${verno} ${commit} ${verdate} ${digest}
echo "-------------------"
