...
echo "* Copied as ${PROD}/sniffer.txt"
cp Exe/sniffer.txt ${PROD}/

echo "* Update version info in ${PROD}/version.csv"
verdate=$(git log -n 1 --pretty=format:'%ai' $verdir | sed 's/ .*//')
digest=$(sha256sum ${PROD}/sniffer.txt | awk '{print $1;}')
echo "-------------------"
cd ${PROD} && python $VERPY sniffer.txt ${verno} ${commit} ${verdate} ${digest}
echo "-------------------"
